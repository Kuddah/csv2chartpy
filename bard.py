        ...
        for i in range(num_charts):
            ...
            with cols[2]:
                if page == "Single Y-axis":
                    ...
                    color = st.selectbox(f'Select chart color for Chart {i+1}:', px.colors.qualitative.Plotly)
                    ...
                    # Allow user to choose chart color for each Y-axis column
                    colors = st.columns(len(y_columns))
                    for j, (y_column, color_col) in enumerate(zip(y_columns, colors)):
                        with color_col:
                            color = st.selectbox(f'Select chart color for {y_column}:', px.colors.qualitative.Plotly)
                            # Create line chart using Plotly Graph Objects
                            fig.add_trace(go.Scatter(x=df[x_column], y=df[y_column], mode='lines', name=y_column, line=dict(color=color)))
